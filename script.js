//a) Write a constructor for the class Movie, which takes a String representing the title of the movie, a String representing the studio, and a String representing the rating as its arguments, and sets the respective class properties to these values.

class movie {
   constructor (title,studio,rating){
    this.title = title;
    this.studio = studio;
    this.rating = rating
}
getmovieDetails(){
    return (`The Movie is ${this.title}, presented by ${this.studio},
    with rating ${this.rating}   `)
}
}
let movies = new movie ("Vikram", "Raj Kamal","PG16",)
console.log (movies.getmovieDetails())

// b) The constructor for the class Movie will set the class property rating to "PG" as default when no rating is provided.

class PGmovie {
    constructor (title,studio,rating){
    this.title = title;
    this.studio = studio;
    if(rating) {
        this.rating = rating
    } else {
        this.rating = 'PG'
    }
    
}
getmovieDetails(){
    
    return (`The Movie is ${this.title}, presented by ${this.studio},
     with rating ${this.rating}`)}}

     let movie1 = new PGmovie ("Vikram", "Raj Kamal","PG13")
     let movie2 = new PGmovie ("Frozen", "Disney",)
     console.log (movie1.getmovieDetails(),movie2.getmovieDetails())


     //c) Write a method getPG, which takes an array of base type Movie as its argument,
     // and returns a new array of only those movies in the input array with a rating of "PG". You may assume the input array is full of Movie instances. The returned array need not full.

    /* class listmovies {
        constructor (object){
         this.object = object;
         
     }
     getmovieDetails(){
         return (`The Movie is ${this.title}, presented by ${this.studio},
         with rating ${this.rating}   `)

       
     }
    
    getPgRatedMovies() {
        return this.object.filter((movie)=>{return movie.rating !== undefined})
     }
}
const listOfMovies = [
    {title:"movie1",rating:"PG13"},
    {title:"movie2",studio:"studio2"},
    {title:"movie3",studio:"studio3",rating:"PG"}
]


console.log(listOfMovies.getPgRatedMovies()) */                      

// d) Write a piece of code that creates an instance of the class Movie with the title “Casino Royale”,
// the studio “Eon Productions”, and the rating “PG­13”

class CasinoRoyal {
    constructor (title,studio,rating)
    {
        this.title = title;
        this.studio = studio;
        this.rating = rating
    }
    getmovieDetails(){
        return (`The Movie is ${this.title}, presented by ${this.studio},
        with rating ${this.rating}   `)
    }
    }
    let data = new CasinoRoyal ("Casino Royal", "Eon Production","PG13",)
    console.log (data.getmovieDetails())